...
loss_percent = 0.1
spread_fraction = 25.0

amount_buy = convert(trade_amount, 'USD', target_coin )
buy_price = get_price(from_coin='USD', to_coin=target_coin)
sell_price = buy_price * (1 + config.tx / spread_fraction + config.tx + config.tx / 2.0 + profit_percent)


print('buy_price =', buy_price )
print('sell_price =', sell_price)
print('target price movement, % = ', (sell_price / buy_price - 1) * 100)
...
if 'id' in res.keys():
    if True:
        ...
        '''
        res = exchange.create_order(symbol=target_coin + 'USD', amount=amount_buy, price=sell_price, side='sell', order_type='limit')
        print(res)
    else:
